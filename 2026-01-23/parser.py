...
import random
from settings import HEADER, DOMAIN, CLIENTS
...
class Parser():
    ...
    def parser(self,pdp_url):
        ...
        session = tls_client.Session(client_identifier=client)
        product_response = session.get(product_url,headers=self.header)
        print(f"[INFO] STATUS CODE = {product_response.status_code}")
